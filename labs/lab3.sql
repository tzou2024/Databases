...
SELECT Fname, LName, Address FROM RENTER WHERE RenterNo IN 
...
SELECT FName, LName, BranchNo FROM STAFF WHERE StaffNo IN 
(SELECT StaffNo FROM PROPERTY WHERE City = 'Cork');

-- 5. Find the names of renters who have viewed properties with at least 3 rooms.
SELECT Fname, Lname FROM RENTER where RenterNo IN 
(SELECT RenterNo FROM VIEWING WHERE PropertyNo IN (SELECT PropertyNo FROM PROPERTY WHERE Rooms > 2));

-- 6. Find the names of staff members who handle properties owned by Woody Allen.
-- SELECT FName, LName from STAFF WHERE STAFFNO = 
SELECT Fname, LName FROM STAFF WHERE STAFFNo IN 
